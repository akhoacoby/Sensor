...
vector<tuple<string, string, string, string, string, string>> System::loadUsers() {
    Csvfile userFile("users.csv");
    vector<tuple<string, string, string, string, string, string>> users;
    ...
    for (const string& line : userFile.getLines()) {
        ...
        string login, password, role, firstName, lastName, email;
        ...
        users.push_back(make_tuple(login, password, role, firstName, lastName, email));
    }
    return users;
}

void System::saveUsers(const vector<tuple<string, string, string, string, string, string>>& users) {
    ofstream ofs("users.csv");
    if (!ofs) {
        cerr << "Error in writing on the csv file " << endl;
        return;
    }

    // Entête
    ofs << "login;password;role;first name;last name;email\n";

    for (const auto& user : users) {
        ofs << get<0>(user) << ";" << get<1>(user) << ";" << get<2>(user) << ";"
            << get<3>(user) << ";" << get<4>(user) << ";" << get<5>(user) << "\n";
    }
}

bool System::createAccount(const string& login,
    ...
    if (login.empty() || password.empty()) {
        cout << "Login or passwordempty." << endl;
        return false;
    }
    ...
    // Sauvegarder tous les utilisateurs (avec l'entête)
    saveUsers(users);
...
bool System::verifyLogin(const string& login,
                         const string& password) {
    auto users = loadUsers();

    for (const auto& user : users) {
        if (get<0>(user) == login && get<1>(user) == password) {
            cout << "Welcom Mr. " << get<2>(user) << endl;
            return true;
        }
    }
